...
def procesar_mensaje(mensaje: str):
    ...
    if mensaje.startswith("PESO:"):
        ...
        try:
            peso = float(mensaje[6:])
            registrar_dispensacion(peso)
        except ValueError as e:
            logger.error(f"Error procesando final: {str(e)}")

def registrar_dispensacion(peso_final: float):
    global system_state
    
    if not system_state["dispensaciones"]:
        peso_inicial = 0.0
    else:
        peso_inicial = system_state["dispensaciones"][-1]["restante"]
    
    peso_dispensado = peso_final - peso_inicial
    
    system_state["dispensaciones"].append({
        "timestamp": time.time(),
        "peso": peso_dispensado,
        "restante": system_state["total_inicial"] - peso_final
    })
...
def interfaz_web():
    peso_restante = system_state["total_inicial"] - sum(
        d["peso"] for d in system_state["dispensaciones"])
    
    tabla = "".join(
        f"<tr><td>{time.strftime('%H:%M:%S', time.localtime(d['timestamp']))}</td>"
    ...
    <html>
        ...
        <body>
            ...
            <div class="data-display">
                <strong>Total restante:</strong> {peso_restante:.1f}g
            </div>
            
            <button onclick="iniciarServido()">Iniciar Servido</button>
